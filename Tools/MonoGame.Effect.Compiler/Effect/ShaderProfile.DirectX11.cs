...
namespace MonoGame.Effect
{
    class DirectX11ShaderProfile : ShaderProfile
    {
        ...
        private static readonly Regex HlslHullShaderRegex = new Regex(@"^hs_(?<major>5)_(?<minor>0)$", RegexOptions.Compiled);
        private static readonly Regex HlslDomainShaderRegex = new Regex(@"^ds_(?<major>5)_(?<minor>0)$", RegexOptions.Compiled);
        ...
        internal override void ValidateShaderModels(PassInfo pass)
        {
            ...
            if (!string.IsNullOrEmpty(pass.hsFunction))
            {
                ...
                if (major != 5)
                    throw new Exception(String.Format("Invalid profile '{0}'. Hull shader '{1}' must use hs_5_0!", pass.hsModel, pass.hsFunction));
            }
            ...
            if (!string.IsNullOrEmpty(pass.dsFunction))
            {
                ...
                if (major != 5)
                    throw new Exception(String.Format("Invalid profile '{0}'. Domain shader '{1}' must use ds_5_0!", pass.dsModel, pass.dsFunction));
            }
        }
